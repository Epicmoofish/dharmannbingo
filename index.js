...
// Data from a file.
let data = [];
const serverSideStorage = "../dharmannbingo/data/db.json";
...
app.post('/api/admin/add/', function (req, res) {
    ...
    data.push(word);
    saveToServer(data);
    res.send({"word":word, "index": data.length-1});
    res.end();
});
...
// TODO: Add your code here.
app.get('/api/admin/word/:index', function (req, res) {
    res.send({"word": data[req.params.index], "index":parseInt(req.params.index)});
    res.end();
});
 ...
 *    response:                  {"word": "...", "index": #}
 */
app.put("/api/admin/word/:index", function(req, res) {
    let word = req.body.word;
    data[req.params.index] = word;
    saveToServer(data);
    res.send({"word":word, "index": parseInt(req.params.index)});
    res.end();
});
// TODO: Add your code here.
app.delete("/api/admin/word/:index", function(req, res) {
...
app.get("/api/player/wordlength/:id", function(req, res) {
    res.send({"length": data[req.params.id].length, "index": parseInt(req.params.id)});
    res.end();
});
...
app.get("/api/player/guess/:id/:letter", function(req, res) {
    ...
    let word = data[id].toUpperCase();
    let letter = req.params.letter.toUpperCase();
    let jsonObj = {"letter": letter, "length": word.length, "index": id, "locations":[]};
    for (let i = 0; i<word.length; i++) {
        if (word[i] == letter) {
            jsonObj.locations.push(i);
        }
    }
    res.send(jsonObj);
    res.end();
